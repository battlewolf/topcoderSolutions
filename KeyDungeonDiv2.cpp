...
int KeyDungeonDiv2::countDoors(vector <int> doorR, vector <int> doorG, vector <int> keys) {
	int t1, t2, n = doorR.size();
	int x = keys[0], y = keys[1], z = keys[2];
	int cnt = 0;
	for(int i = 0; i < n; i++)
	{
		bool f = 0;
		z = keys[2];
		if(x == 0 && y == 0 && z >= (doorR[i] + doorG[i])) {cnt++;continue;}
		if(x >= doorR[i] && y >= doorG[i]) cnt++;
		if(x < doorR[i])
		{
			if(z + x < doorR[i]) continue;
			else
			{
				f = 1;
				z = doorR[i] - z;
